...
split : List (Var × UExp) → (List Var) × (List UExp)
split [] = [] , []
split (x ∷ x₁) = (fst x ∷ fst (split x₁)) , ((snd x ∷ snd (split x₁)))
...
data _⇓_ : {H₁ H₂ : Heap} → {E₁ E₂ : Exp} → H₁ entails E₁ → H₂ entails E₂ → Set where
  ...
  var_red : {Γ Θ Δ : Heap} {y : Var} {e x z : Exp} →
    Γ ⊢ e ⇓ Δ ⊢ z →
    Δ ⊢ x ⇓ Δ ⊢ hat z
